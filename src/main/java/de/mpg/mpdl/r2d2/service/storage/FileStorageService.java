...
public class FileStorageService {
	...
	public void creatreManifest(String segmentContainer, String segmentPath, String manifestContainer,
			String manifest) {
		...
		ObjectList list = swiftApi.getObjectApi(swiftProperties.getRegion(), segmentContainer)
				.list(ListContainerOptions.Builder.path(segmentPath));
		list.forEach(so -> {
			long size = so.getPayload().getContentMetadata().getContentLength();
			Segment s = Segment.builder().path(segmentContainer + "/" + so.getName()).etag(so.getETag()).sizeBytes(size)
					.build();
			parts.add(s);
		});
