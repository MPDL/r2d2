...
public class FileUploadController {
	...
	public ResponseEntity<String> initializeUpload(@PathVariable("dataset") String dataset,
			@RequestHeader("Authorization") String token, @RequestHeader("X-File-Name") String fileName,
			@RequestHeader("X-File-Total-Chunks") int totalChunks, Principal prinz)
	...
	public ResponseEntity<File> uploadPart(@PathVariable("dataset") String dataset, @PathVariable("file") String file,
			@RequestHeader("Authorization") String token, @RequestHeader("X-File-Chunk-Number") int part,
			HttpServletRequest request, Principal prinz) throws R2d2ApplicationException, R2d2TechnicalException {
		...
		if (part == Integer.MAX_VALUE) {
			...
			try {
				...
				while (iterator.hasNext()) {
					...
					if (!item.isFormField()) {
						file2update = storageService.upload(file, part, item.openStream().readAllBytes(), item.getContentType(),
	...
	ResponseEntity<?> download(@PathVariable("dataset") String dataset, @PathVariable("file") String file,
		...
		InputStreamResource inputStreamResource = new InputStreamResource(
				storageService.download(dataset, file, Utils.toCustomPrincipal(prinz)));
