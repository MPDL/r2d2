...
import de.mpg.mpdl.r2d2.util.JsonObjectMapperFactory;
...
public abstract class ElasticSearchGenericDAOImpl<E> implements GenericDaoEs<E> {
	...
	public String create(String id, E entity) throws R2d2TechnicalException {
		try {
			...
			IndexRequest ir = new IndexRequest(indexName).id(id)
					.source(mapper.writeValueAsBytes(applyCustomValues(entity)), XContentType.JSON);
		...
		try {
			...
			if (getSourceExclusions() != null && getSourceExclusions().length > 0) {
				if (ssb.fetchSource() == null) {
					...
					String[] both = Stream
							.concat(Arrays.stream(getSourceExclusions()),
									(excludes != null ? Arrays.stream(excludes) : Arrays.stream(new String[0])))
							.toArray(String[]::new);
	...
	public SearchResult<E> getSearchRetrieveResponseFromElasticSearchResponse(SearchResponse sr, Class<E> clazz)
			throws IOException {
	...
	public Map<String, ElasticSearchIndexField> getIndexFields() throws R2d2TechnicalException {
		...
		try {
			...
			if (resp.mappings().isEmpty() == false) { // SP: avoiding NullPointerException
				...
				Map<String, ElasticSearchIndexField> map = ElasticSearchIndexField.Factory
						.createIndexMapFromElasticsearch(mmd);
